...
def cm_3dtrain_defaults():
    """
    Arguments default for 3d points cloud training
    """
    return dict(
        train_data_dir = "data/proxd_train",
        valid_data_dir = "data/proxd_valid",
        load_ckpt = None,
        posa_path = "training/posa/model_ckpt/best_model_recon_acc.pt",
        out_dir = "training/",
...
#     diffusion = create_karras_diffusion(model_and_diffusion_defaults())
#     return model, diffusion

def create_karras_diffusion(res):
    return KarrasDenoiser(
        sigma_data=0.5, 
        sigma_max = res['sigma_max'], 
        sigma_min = res['sigma_min'], 
        rho = res['rho'], 
        weight_schedule=res['weight_schedule'],
        lambda_vel=res['lambda_vel'],
        lambda_rcxyz=res['lambda_rcxyz'],
        lambda_fc=res['lambda_fc'],
        lambda_cat=res['lambda_cat']
    )
